...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        "Read relevant code before changing it and keep changes tightly scoped to the request. Always use `read_file` before `edit_file` and copy `old_string` verbatim from its output.".to_string(),
