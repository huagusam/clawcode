...
If `new_string` is shorter than `old_string`, confirm you are intentionally removing content.",
        ...
        ToolSpec {
         ...
         /*
          * `SendUserMessage` delivers a free-form message to the user outside the
          * normal assistant reply stream. It is the canonical "push" channel for
          * surfacing results, summaries, or file locations the user should see
          * immediately, and it does not consume a tool-result turn. The model may
          * call it at any point; it requires only ReadOnly permission because it
          * never reads or mutates the filesystem. `message` is the required body
          * (markdown supported); `attachments` is an optional list of file paths
          * or resource references rendered alongside the message.
          *
          * Supported in both UI and terminal; could be adapted into an
          * AskUserQuestion tool (similar to opencode's built-in tool).
          */
        // ToolSpec {
        //     name: "SendUserMessage",
        //     description: "Send a message to the user. `message` is required (markdown supported); `attachments` is an optional array of file paths or resource references.",
        //     input_schema: json!({
        //         "type": "object",
        //         "properties": {
        //             "message": { "type": "string" },
