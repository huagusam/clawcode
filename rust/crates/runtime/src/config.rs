...
/// Hook command lists grouped by lifecycle event name (e.g. `PreToolUse`,
/// `SessionStart`, `Stop`). Using a map lets any Claude Code hook event be
/// registered and dispatched without a per-event field.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let claude_settings_path = self.config_home.parent().map_or_else(
        ...
        // Walk ancestors (outermost first = lowest priority) for project configs.
        // Skip the user's home directory and anything above it: config files
        // at or above the home are user-scope, not project-scope. Without
        // this skip the walk climbs to the drive root and picks up
        // `~/.claw/settings.json` and `~/.claude/settings.json` as if they
        // were project-level — leaking the developer's personal config into
        // every project's merge.
        //
        // Canonicalize both sides so 8.3 short names (e.g. `INCRED~1`) on
        // Windows don't fool the textual comparison. If canonicalization
        // fails (e.g. cwd was deleted mid-load), fall back to the textual
        // path and apply the same boundary check — at worst, an obscure
        // edge case keeps the old leak; the common case is fixed.
        //
        // Strip the Windows verbatim-path prefix (`\\?\`) that
        // `canonicalize()` injects so the stored ConfigEntry paths are
        // human-readable and consistent.
        let canonical_home = user_home_dir()
            .and_then(|h| h.canonicalize().ok())
            .map(strip_verbatim_prefix);
        let canonical_cwd = self.cwd.canonicalize().ok().map(strip_verbatim_prefix);
        let cwd_ancestors: Vec<PathBuf> = canonical_cwd
            .as_ref()
            .map(|c| c.ancestors().map(|a| a.to_path_buf()).collect())
            .unwrap_or_else(|| self.cwd.ancestors().map(|a| a.to_path_buf()).collect());
        let fallback_home = user_home_dir();
        for ancestor in cwd_ancestors.iter().rev() {
            let at_or_above_home = canonical_home
                .as_ref()
                .map_or_else(
                    || {
                        fallback_home
                            .as_deref()
                            .map_or(false, |h| h.starts_with(ancestor))
                    },
                    |home| home.starts_with(ancestor),
                );
            if at_or_above_home {
                // The home directory itself and any of its ancestors (up to
                // the drive root) hold user-scope config. Skip without
                // breaking: we still need to process ancestors *below* home
                // in the outermost-first walk order, e.g. the cwd itself.
                continue;
            }
            let project_paths = [
                (ConfigSource::Project, ancestor.join(".claw").join("settings.json")),
                (ConfigSource::Project, ancestor.join(".claude").join("settings.json")),
            ];
            for (source, path) in &project_paths {
                if path.is_file() {
    ...
    pub fn load(&self) -> Result<RuntimeConfig, ConfigError> {
        ...
        for entry in self.discover() {
            ...
            if entry.source != ConfigSource::Plugin && !validation.is_ok() {
...
#[must_use]
/// Strip the Windows verbatim-path prefix `\\?\` from a canonicalized path,
/// including UNC paths (`\\?\UNC\...` → `\\...\`).
/// Uses the `dunce` crate for correct handling of all Windows path variants.
...
#[must_use]
/// Returns the user's home directory (`%USERPROFILE%` on Windows,
/// `$HOME` elsewhere), or `None` if the env var is unset.
///
/// Used as the project-ancestor-walk boundary in [`ConfigLoader::discover`]:
/// config files at or above the home are user-scope, not project-scope.
...
impl RuntimeHookConfig {
    ...
    pub fn new(
        ...
        if !pre_tool_use.is_empty() {
            events.insert("PreToolUse".to_string(), pre_tool_use);
        }
        if !post_tool_use.is_empty() {
            events.insert("PostToolUse".to_string(), post_tool_use);
        }
        if !post_tool_use_failure.is_empty() {
            events.insert("PostToolUseFailure".to_string(), post_tool_use_failure);
        }
        Self { events }
    }

    /// Build directly from an event-name -> commands map (used when bridging
    /// plugin manifests, which may carry arbitrary Claude Code events).
    #[must_use]
    pub fn from_events(events: BTreeMap<String, Vec<String>>) -> Self {
        Self { events }
    }
    ...
    pub fn pre_tool_use(&self) -> &[String] {
        self.commands_for("PreToolUse")
    }

    #[must_use]
    pub fn post_tool_use(&self) -> &[String] {
        self.commands_for("PostToolUse")
    }

    #[must_use]
    pub fn post_tool_use_failure(&self) -> &[String] {
        self.commands_for("PostToolUseFailure")
    }
    ...
    pub fn extend(&mut self, other: &Self) {
        for (event, commands) in &other.events {
            let entry = self.events.entry(event.clone()).or_default();
...
fn merge_mcp_servers(
    ...
    for key in &["mcpServers", "mcp"] {
        let Some(mcp_section) = root.get(*key) else {
            continue;
        };
        let servers = expect_object(mcp_section, &format!("{}: {key}", path.display()))?;
        for (name, value) in servers {
            // Skip servers explicitly marked as disabled.
...
fn parse_optional_hooks_config_object(
    ...
    for event in hooks.keys() {
        if let Some(commands) = optional_string_array(hooks, event, context)? {
            if !commands.is_empty() {
                events.insert(event.clone(), commands);
            }
        }
    }
    Ok(RuntimeHookConfig::from_events(events))
}
...
fn parse_optional_plugin_config(root: &JsonValue) -> Result<RuntimePluginConfig, ConfigError> {
    ...
    // Extract known config sub-fields.
    ...
    //   "plugins": {
    //     "plugin-name": { "enabled": true }     // standard
    //     "plugin-name": true                     // compact
    //   }
    // Known config sub-fields are skipped; remaining keys are treated as plugin entries.
    let known_plugin_fields = [
        "externalDirectories",
        "installRoot",
        "registryPath",
        "bundledRoot",
    ...
    for (key, value) in plugins {
        ...
        let enabled = match value {
            // Standard format: { "enabled": true/false }
            JsonValue::Object(obj) => {
                match obj.get("enabled").and_then(JsonValue::as_bool) {
                    Some(e) => e,
                    // Object without "enabled" field — not a plugin entry, skip.
                    None => continue,
                }
            }
            // Compact format: true/false directly (disambiguated by @ in name)
            JsonValue::Bool(b) if key.contains('@') => *b,
            // Not a plugin entry, skip.
            _ => continue,
        };
        // Lower priority than enabledPlugins — insert only if not already set.
        config.enabled_plugins.entry(key.clone()).or_insert(enabled);
    }
...
fn parse_optional_permission_mode(
    ...
    if let Some(mode) = object
        ...
        .and_then(JsonValue::as_str)
    {
        return parse_permission_mode_label(mode, "merged settings.permissions.defaultMode")
            .map(Some);
    }
    if let Some(mode) = object.get("defaultMode").and_then(JsonValue::as_str) {
        return parse_permission_mode_label(mode, "merged settings.defaultMode").map(Some);
    }
    Ok(None)
}
...
pub fn parse_mcp_server_config(
    ...
    // "local" is the opencode/claw standard type — equivalent to stdio
    let server_type = if server_type == "local" { "stdio" } else { server_type };
    match server_type {
        "stdio" => {
            let (command, args) = parse_command_field(object, context)?;
...
/// Parses `"command"` as either a string or an array of strings.
///
/// - `"command": "uvx"` → command=`"uvx"`, args from separate `"args"` field
/// - `"command": ["chrome-devtools-mcp", "--port", "9222"]` →
///   command=`"chrome-devtools-mcp"`, args=`["--port", "9222"]`
fn parse_command_field(
    object: &BTreeMap<String, JsonValue>,
    context: &str,
) -> Result<(String, Vec<String>), ConfigError> {
    let cmd_val = object.get("command").ok_or_else(|| {
        ConfigError::Parse(format!("{context}: missing required field 'command'"))
    })?;
    match cmd_val {
        JsonValue::String(s) => Ok((s.clone(), optional_string_array(object, "args", context)?.unwrap_or_default())),
        JsonValue::Array(arr) => {
            let command = arr
                .first()
                .and_then(JsonValue::as_str)
                .ok_or_else(|| {
                    ConfigError::Parse(format!(
                        "{context}: 'command' array must have at least one string element"
                    ))
                })?;
            let args: Vec<String> = arr.iter().skip(1).filter_map(JsonValue::as_str).map(String::from).collect();
            Ok((command.to_string(), args))
        }
        other => Err(ConfigError::Parse(format!(
            "{context}: 'command' must be a string or array of strings, got {other:?}"
        ))),
    }
}
...
mod tests {
    use super::{
        deep_merge_objects, parse_permission_mode_label, user_home_dir, ConfigLoader,
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        fs::write(
            ...
            r#"{"model":"sonnet","env":{"A2":"1"},"hooks":{"PreToolUse":["base"]},"mcpServers":{"home":{"command":"uvx","args":["home"]}},"permissions":{"defaultMode":"plan","allow":["Read"],"deny":["Bash(rm -rf)"]}}"#,
        )
        .expect("write user settings");
        fs::write(
            cwd.join(".claw").join("settings.json"),
            r#"{"env":{"C":"3"},"hooks":{"PostToolUse":["project"],"PostToolUseFailure":["project-failure"]},"permissions":{"ask":["Edit"]},"mcpServers":{"project":{"command":"uvx","args":["project"]}},"model":"opus","permissionMode":"acceptEdits"}"#,
        )
    ...
    #[test]
    fn discover_ancestor_walk_stops_at_user_home_boundary() {
        // Regression: previously `discover()` walked all ancestors of cwd
        // to the drive root, picking up the developer's real
        // `~/.claw/settings.json` and `~/.claude/settings.json` as if they
        // were project-scope. The fix stops the walk at the user's home
        // (USERPROFILE on Windows, $HOME elsewhere). We exercise both
        // directions: a fake project below the real home should NOT
        // surface entries that live at or above the home, and a fake
        // project above the real home (different drive / drive root) is
        // outside the leak path entirely.
        let user_home = match user_home_dir() {
            Some(h) => h,
            None => return, // env var not set — nothing to test
        };
        let canonical_home = match user_home.canonicalize() {
            Ok(h) => h,
            Err(_) => return, // home doesn't exist on this host
        };

        // Case 1: a fake project at the home root itself. The walk should
        // see the home as the boundary, return zero project-scope entries.
        let project_at_home = canonical_home.join("__claw_test_project_at_home__");
        fs::create_dir_all(&project_at_home).expect("create project at home");
        let entries = ConfigLoader::new(&project_at_home, &canonical_home)
            .discover();
        let project_entries_at_home: Vec<_> = entries
            .iter()
            .filter(|e| e.source == ConfigSource::Project)
            .collect();
        assert!(
            project_entries_at_home.is_empty(),
            "walk at home root should yield no project entries, got: {:?}",
            project_entries_at_home
        );
        fs::remove_dir_all(&project_at_home).expect("cleanup at-home project");

        // Case 2: a fake project nested inside the home (e.g. `~/foo`).
        // The walk should climb `~/foo` -> `~` and stop — `~/.claw/...`
        // and `~/.claude/...` are user-scope, already represented as
        // `ConfigSource::User` / `ConfigSource::Plugin` entries.
        let project_inside_home = canonical_home.join("__claw_test_nested__");
        fs::create_dir_all(&project_inside_home).expect("create nested project");
        let entries = ConfigLoader::new(&project_inside_home, &canonical_home)
            .discover();
        for entry in &entries {
            if entry.source == ConfigSource::Project {
                assert!(
                    !entry.path.starts_with(&canonical_home),
                    "project entry leaked from above/below home boundary: {:?}",
                    entry.path
                );
            }
        }
        fs::remove_dir_all(&project_inside_home).expect("cleanup nested project");
    }
    ...
    #[test]
    fn parses_mcp_key_format_with_local_type_and_command_array() {
        // Standard opencode/claw format: "mcp" key, "type": "local", "command" as array
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".claw");
        fs::create_dir_all(&home).expect("home config dir");
        ...
        let server = loaded
            .mcp()
            .get("chrome-devtools")
            .expect("chrome-devtools server should exist");
        assert_eq!(server.transport(), McpTransport::Stdio);
        match &server.config {
            McpServerConfig::Stdio(config) => {
                assert_eq!(config.command, "chrome-devtools-mcp");
                assert_eq!(config.args, vec!["--port", "9222"]);
            }
    ...
    #[test]
    fn skip_mcp_servers_disabled_by_enabled_field() {
        // enabled: false should skip the server entirely
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".claw");
        fs::create_dir_all(&home).expect("home config dir");
        ...
        assert!(
            loaded.mcp().get("enabled-server").is_some(),
            "enabled-server should be present"
        );
        assert!(
            loaded.mcp().get("disabled-server").is_none(),
    ...
    #[test]
    fn parses_mcp_alongside_mcp_servers() {
        // Both "mcp" and "mcpServers" keys should be parsed
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".claw");
        fs::create_dir_all(&home).expect("home config dir");
        ...
        .expect("write combined settings");
        ...
        assert!(
            loaded.mcp().get("server-a").is_some(),
            "server-a from 'mcp' key should exist"
        );
        assert!(
            loaded.mcp().get("server-b").is_some(),
            "server-b from 'mcpServers' key should exist"
        );
    ...
    #[test]
    fn parses_plugin_entries_in_plugins_object_standard_format() {
        // Standard format: "plugins": { "name@scope": { "enabled": true/false } }
        // coexists with known config sub-fields.
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".claw");
        fs::create_dir_all(&home).expect("home config dir");
        ...
        fs::write(
            ...
            r#"{
              "plugins": {
                "frontend-design@official": { "enabled": true },
                "superpowers@official": { "enabled": false },
                "externalDirectories": ["./ext"],
                "installRoot": "cache"
              }
            }"#,
        )
        ...
        let enabled = loaded.plugins().enabled_plugins();
        assert_eq!(enabled.get("frontend-design@official"), Some(&true));
        assert_eq!(enabled.get("superpowers@official"), Some(&false));
        assert_eq!(
            loaded.plugins().external_directories(),
            &["./ext".to_string()]
        );
        assert_eq!(loaded.plugins().install_root(), Some("cache"));

        fs::remove_dir_all(root).expect("cleanup temp dir");
    }

    #[test]
    fn parses_plugin_entries_compact_format_with_at_symbol() {
        // Compact format: "plugins": { "name@scope": true/false }
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".claw");
        fs::create_dir_all(&home).expect("home config dir");
        ...
        let enabled = loaded.plugins().enabled_plugins();
        assert_eq!(enabled.get("tool-guard@builtin"), Some(&true));
        assert_eq!(enabled.get("sample-plugin@external"), Some(&false));

        fs::remove_dir_all(root).expect("cleanup temp dir");
    }

    #[test]
    fn plugin_entries_in_plugins_have_lower_priority_than_enabled_plugins() {
        // enabledPlugins should override plugin entries in "plugins" object
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".claw");
        fs::create_dir_all(&home).expect("home config dir");
        ...
        // enabledPlugins takes priority — should be false
        assert_eq!(
            loaded.plugins().enabled_plugins().get("conflict@test"),
            Some(&false)
        );
