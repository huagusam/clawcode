...
use crate::boundary::{
    canonicalize_maybe_missing, classify_boundary, BoundaryCheck, PolicyOutcome, BoundaryPolicy,
};
...
pub fn normalize_path_for_output(path: &Path) -> String {
    dunce::simplified(path).as_os_str().to_string_lossy().replace('\\', "/")
}
...
pub fn new_file(path: &str, content: &str, force: bool) -> io::Result<WriteFileOutput> {
    ...
    Ok(WriteFileOutput {
        ...
        bytes_written,
        lines_written,
        content_preview: if content.len() <= CONTENT_PREVIEW_SKIP_THRESHOLD { preview_for(content) } else { None },
...
pub fn grep_search(input: &GrepSearchInput) -> io::Result<GrepSearchOutput> {
    ...
    for file_path in collect_search_files(&base_path)? {
        ...
        if output_mode == "count" {
            ...
            if count > 0 {
                filenames.push(normalize_path_for_output(&file_path));
                total_matches += count;
            }
            continue;
        }
        ...
        if output_mode == "content" {
            for index in matched_lines {
                ...
                for (current, line) in lines.iter().enumerate().take(end).skip(start) {
                    let normalized = normalize_path_for_output(&file_path);
                    let prefix = if input.line_numbers.unwrap_or(true) {
                        format!("{normalized}:{}:", current + 1)
                    } else {
                        format!("{normalized}:")
                    };
...
pub fn read_file_with_policy(
    ...
    read_file(canonical_path.to_string_lossy().as_ref(), offset, limit, full)
}
...
pub fn edit_file_with_policy(
    ...
    edit_file(canonical_path.to_string_lossy().as_ref(), old_string, new_string, replace_all, expected_checksum)
}
...
pub fn grep_search_with_policy(
    ...
    if matches!(
        ...
        return Err(io::Error::new(
            ...
            format!("search root {} escapes workspace boundary", base_path.display()),
        ));
    }
...
mod tests {
    ...
    use crate::boundary::{BoundaryDecision, Prompter, PrompterError, BoundaryPolicy};
    ...
    fn edit_file_matches_across_crlf_line_endings() {
        ...
        assert!(payload.content_preview.as_deref().unwrap().contains("SECOND"));
        let on_disk = std::fs::read_to_string(&path).expect("read back");
        assert_eq!(on_disk, "first\nSECOND\nthird\n");
        let _ = std::fs::remove_file(&path);
    }
