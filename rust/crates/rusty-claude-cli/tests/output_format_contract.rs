...
fn agents_command_emits_structured_agent_entries_when_requested() {
    ...
    let user_agents = home.join(".claw").join("agents");
    let isolated_config = root.join("config-home");
    let isolated_codex = root.join("codex-home");
    ...
    assert_eq!(parsed["agents"][1]["name"], "planner");
    assert_eq!(parsed["agents"][1]["active"], false);
    assert_eq!(parsed["agents"][1]["shadowed_by"]["id"], "project_claw");
    assert_eq!(parsed["agents"][2]["name"], "verifier");
    assert_eq!(parsed["agents"][2]["active"], true);
}
...
fn doctor_and_resume_status_emit_json_when_requested() {
    ...
    assert_eq!(
        ...
        "https://github.com/huagusam/clawcode"
    );
    assert_eq!(
...
fn write_upstream_fixture(root: &Path) -> PathBuf {
    let upstream = root.join("clawcode");
