...
fn resume_latest_restores_the_most_recent_managed_session() {
    ...
    fs::create_dir_all(&config_home).expect("config home should exist");
    let project_dir = fs::canonicalize(&project_dir).unwrap_or(project_dir);
    let config_home = fs::canonicalize(&config_home).unwrap_or(config_home);
    let config_home_str = config_home.to_str().expect("utf8 path");

    std::env::set_var("CLAW_CONFIG_HOME", config_home_str);
    let store =
        runtime::SessionStore::from_cwd(&project_dir).expect("session store should build");
    ...
    let output = run_claw_with_env(
        ...
        &[("CLAW_CONFIG_HOME", config_home_str)],
    );
