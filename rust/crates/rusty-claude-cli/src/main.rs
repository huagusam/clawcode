...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    let mut workspace_policy_override: Option<runtime::WorkspacePolicyKind> = None;
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--workspace-policy" => {
                ...
                let kind = runtime::WorkspacePolicyKind::parse(value).ok_or_else(|| {
                    format!(
    ...
    let resolved_kind = workspace_policy_override
        .or_else(|| {
            env::var("CLAW_WORKSPACE_POLICY")
                .ok()
                .and_then(|v| runtime::WorkspacePolicyKind::parse(&v))
        })
        .unwrap_or(runtime::WorkspacePolicyKind::Strict);
    let resolved_policy = match resolved_kind {
        runtime::WorkspacePolicyKind::Strict => runtime::WorkspacePolicy::Strict,
        runtime::WorkspacePolicyKind::Allow => runtime::WorkspacePolicy::Allow,
        runtime::WorkspacePolicyKind::Prompt => {
            use std::collections::BTreeSet;
            use std::sync::{Arc, Mutex};
            runtime::WorkspacePolicy::Prompt {
                prompter: Arc::new(runtime::StdinPrompter::default()),
        ...
        match editor.read_line_interactive()? {
            input::ReadOutcome::Submit(input) => {
                ...
                if let Some((skill_name, skill_args)) = resolve_bare_skill_name(&cwd, &trimmed) {
                    ...
                    cli.record_prompt_history(&trimmed);
                    cli.run_turn_forced("Skill", &tool_input, &trimmed)?;
                    continue;
                }
                    ...
                    detect_mentioned_agent(&trimmed, &plugin_agents)
                {
                    ...
                    editor.push_history(input);
                    cli.record_prompt_history(&trimmed);
                    cli.run_turn_forced("Agent", &tool_input, &trimmed)?;
                    continue;
                }
                resolved_input = resolve_mentions(&resolved_input, &plugin_agents);
                editor.push_history(input);
                cli.record_prompt_history(&trimmed);
                cli.run_turn(&resolved_input)?;
            }
                ...
                match execute_tool("undo", &input) {
                    Ok(output) => {
                        ...
                        if let Ok(uo) = serde_json::from_str::<UndoOutput>(&output) {
                            ...
                            if !uo.diff_path.is_empty() {
                                println!("  {}: {}", "diff".dim(), uo.diff_path.dim());
                            }
                        } else {
                            println!("{output}");
                        }
                ...
                if cache.last_ptr == msg_ptr && cache.last_len <= msg_len {
                    ...
                    if msg_len > cache.last_len {
                        let (delta_inputs, delta_cached) =
                            convert_messages_inner(&request.messages[cache.last_len..], None, None);
                    ...
                    full_convert_and_cache_cli(&mut self.message_cache, &request, msg_ptr, msg_len)
                }
            } else {
                full_convert_and_cache_cli(&mut self.message_cache, &request, msg_ptr, msg_len)
            }
                    ...
                    if block_has_thinking_summary {
                        ...
                        if !accumulated_thinking.is_empty() {
                            ...
                            if !clean.trim().is_empty() {
                                ...
                                events.push(AssistantEvent::Thinking(thinking_text));
                            }
                        }
                    }
                    block_has_thinking_summary = false;
                ...
                ApiStreamEvent::MessageStop(_) => {
                    ...
                    if block_has_thinking_summary {
                        ...
                        if !accumulated_thinking.is_empty() {
                            ...
                            if !clean.trim().is_empty() {
                                events.push(AssistantEvent::Thinking(clean));
                            } else if !accumulated_thinking.trim().is_empty() {
                                events.push(AssistantEvent::Thinking(accumulated_thinking.clone()));
                            }
...
fn full_convert_and_cache_cli(
    ...
    let (msgs_arc, vals) =
        convert_messages_cached(&request.messages, image_cache.as_deref(), image_store);
...
/// without extracting or executing them as tools.
fn strip_tool_xml_markup(text: &str) -> String {
    let mut result = String::with_capacity(text.len());
    let mut remaining = text;
    while let Some(tag_start) = remaining.find('<') {
        let after = &remaining[tag_start + 1..];
        if let Some(close_name) = after.find('>') {
            let tag_content = &after[..close_name].trim();
            let close_tag: Option<&str> = if tag_content.eq_ignore_ascii_case("tool_call") {
                Some("</tool_call>")
            } else if tag_content.starts_with("function=")
                || tag_content.eq_ignore_ascii_case("function")
            {
                Some("</function>")
            } else if tag_content.starts_with("parameter=") {
                Some("</parameter>")
            } else {
                None
            };
            ...
            if let Some(close_tag) = close_tag {
                let content_after_open = tag_start + 1 + close_name + 1;
                if let Some(end_rel) = remaining[content_after_open..].find(close_tag) {
                    // Write everything BEFORE the tag, then skip the entire tag
                    result.push_str(&remaining[..tag_start]);
                    let end = content_after_open + end_rel + close_tag.len();
                    remaining = &remaining[end..];
                    continue;
                }
            }
        }
        result.push_str(&remaining[..tag_start + 1]);
...
fn format_tool_result(name: &str, output: &str, is_error: bool) -> String {
    ...
    match name {
        ...
        "SendUserMessage" | "Brief" => format_brief_result(icon, &parsed),
    ...
    match block {
        ...
        OutputContentBlock::Thinking { thinking, .. } => {
            if streaming_tool_input && thinking.is_empty() {
                ...
                let display_text = if clean.trim().is_empty() {
                    &thinking
                } else {
                    &clean
                };
                ...
                let rendered =
                    TerminalRenderer::new().render_reasoning_block(&with_tool_ansi, width, false);
                write!(out, "{rendered}")
                    .and_then(|()| out.flush())
                    .map_err(|error| RuntimeError::new(error.to_string()))?;
        ...
        OutputContentBlock::RedactedThinking { .. } => {
            write!(
                out,
                "{}",
                reasoning_summary(None, true, TerminalRenderer::new().color_theme())
            )
            .and_then(|()| out.flush())
            .map_err(|error| RuntimeError::new(error.to_string()))?;
...
UU conflicted.rs",
    ...
    fn converts_tool_roundtrip_messages() {
        ...
        let converted = super::convert_messages(&messages, None, None);
