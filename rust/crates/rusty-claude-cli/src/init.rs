...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    lines.push("- Keep shared defaults in `.claw/settings.json` or `.claude/settings.json`.".to_string());
...
mod tests {
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join(".gitignore"), ".claw/sessions/\n").expect("write gitignore");
    ...
    fn artifacts_with_status_partitions_fresh_and_idempotent_runs() {
        ...
        assert_eq!(
            ...
            "idempotent init should place both artifacts in skipped[]"
        );
        assert!(
