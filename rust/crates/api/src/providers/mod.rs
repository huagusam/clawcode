...
/// Look up `key` in a `.env` file located in the Claw config directory.
/// The config directory is `$CLAW_CONFIG_HOME` when set, otherwise
/// `~/.claw` (Unix) or `~\.claw` (Windows).
/// Returns `None` when the file is missing, the key is absent, or the value
/// is empty.
pub(crate) fn dotenv_value(key: &str) -> Option<String> {
    let config_home = if let Some(custom) = std::env::var_os("CLAW_CONFIG_HOME") {
...
/// Load `~/.claw/.env` into the process environment at startup.
...
pub fn load_env_file_to_process() {
    ...
    let Some(values) = load_dotenv_file(&config_home.join(".env")) else { return };
