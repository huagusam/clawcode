...
fn ts_ymd_hm(secs: u64) -> (u32, u32, u32, u32, u32) {
    ...
    let era = if z >= 719468 { (z - 719468) as i64 / 146097 } else { 0 };
    ...
    let m = mp + 3 - if mp >= 10 { 12 } else { 0 };
    let y = y + if m <= 2 { 1 } else { 0 };
    let r = secs % 86400;
    let h = r / 3600;
    let mi = (r % 3600) / 60;
    (y as u32, m as u32, d as u32, h as u32, mi as u32)
}

/// Write a diff file under `~/.claw/diffs/d{YYYYMMDD}/diff_{HHMM}.patch`.
...
fn write_diff_file(
    ...
    let date_dir = format!("d{y:04}{m:02}{d:02}");
    let dir = diffs_root.join(&date_dir);
    std::fs::create_dir_all(&dir).ok()?;

    let base = format!("diff_{h:02}{min:02}");
    let filename = if !dir.join(format!("{base}.patch")).exists() {
        format!("{base}.patch")
    } else {
        let mut counter = 1;
        loop {
...
fn delete_diff_file(diff_path: &Path) {
...
/// Find the newest `.patch` file under `diffs_root/d{YYYYMMDD}/`.
fn find_latest_diff(diffs_root: &Path) -> Option<PathBuf> {
    let mut dirs: Vec<_> = std::fs::read_dir(diffs_root).ok()?
        .filter_map(|e| e.ok())
    ...
    for dir in &dirs {
        let mut patches: Vec<_> = std::fs::read_dir(dir.path()).ok()?
            .filter_map(|e| e.ok())
            .filter(|e| e.file_name().to_string_lossy().ends_with(".patch"))
            .collect();
        patches.sort_by(|a, b| b.file_name().cmp(&a.file_name()));
        if let Some(latest) = patches.into_iter().next() {
...
/// Resolve a user-supplied diff name against the date-dir layout.
///
/// 1. Bare name like `diff_2141.patch`         → search date dirs (newest first)
/// 2. Relative path like `d20260722/diff_2141.patch` → under `diffs_root`
/// 3. Absolute / existing path                  → backward compat
fn resolve_diff_path(diffs_root: &Path, input_path: &str) -> Result<PathBuf, String> {
    ...
    // Relative path with separator (e.g. `d20260722/diff_2141.patch`)
    if name.contains('/') || name.contains('\\') {
        let candidate = diffs_root.join(name);
        if candidate.exists() {
            return Ok(candidate);
        }
        return Err(format!("Diff file not found: {name}"));
    }

    // Bare filename — search newest-to-oldest date directories
...
///
/// * Bare `/undo` — auto-discovers the newest `.patch` file.
/// * `/undo diff_2141.patch` — matches the newest date dir containing it.
/// * `/undo d20260722/diff_2141.patch` — exact path under `~/.claw/diffs/`.
fn run_undo(input: UndoInput) -> Result<String, String> {
    let diffs_root = default_config_home().join("diffs");

    // 0. Resolve diff path
    let diff_path = {
        if input.diff_path.is_empty() {
            find_latest_diff(&diffs_root)
                .ok_or_else(|| "Nothing to undo — no diff files found.".to_string())?
        } else {
            resolve_diff_path(&diffs_root, &input.diff_path)?
        }
    };

    let canonical = diff_path
        .canonicalize()
        .map_err(|e| format!("Invalid diff path: {e}"))
        .map(|p| dunce::simplified(&p).to_path_buf())?;
    let diffs_root_canonical = diffs_root
        .canonicalize()
    ...
    let diff: serde_json::Value =
        serde_json::from_str(&diff_content).map_err(|e| format!("Invalid diff file: {e}"))?;
